...
function Step2interview({ interviewData, onFinish }) {
  ...
  const waitForVoices = () =>
    new Promise((resolve) => {
      ...
      const fn = () => {
        window.speechSynthesis.removeEventListener("voiceschanged", fn);
        resolve();
      };
  ...
  useEffect(() => {
    const run = async () => {
      if (isIntroPhase) {
        ...
        await speakText(
          `Hi ${userName}, great to meet you. I hope you're ready.`,
        );
        await speakText(
          `I'll ask you some questions. Answer naturally. Let's begin.`,
        );
  ...
  useEffect(() => {
    if (!isIntroPhase) answerInputRef.current?.focus();
  ...
  useEffect(() => {
    ...
    if (timeLeft === 0) handleSubmitAnswer();
  }, [timeLeft]);
  ...
  return (
    <motion.section
      ...
      initial="hidden"
      animate="show"
      className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/20 px-4 py-6 sm:px-6 lg:px-8"
    >
      <div className="mx-auto grid w-full max-w-7xl gap-6 lg:grid-cols-[1.05fr_1fr]">
        {/* AI Panel */}
        <motion.aside
          variants={panelVariants}
          className="glass-card rounded-2xl p-5 shadow-lg shadow-slate-200/40 sm:p-6"
        >
          <div className="flex h-full flex-col gap-5">
            <div className="flex items-center justify-between">
              <p className="rounded-full border border-indigo-100 bg-gradient-to-r from-indigo-50 to-violet-50 px-3 py-1 text-xs font-semibold tracking-wide text-indigo-600 shadow-sm">
                ✦ LIVE AI INTERVIEWER
              </p>
              <div className="flex items-center gap-2 text-xs text-slate-400">
                <motion.span
                  animate={isAiPlaying ? { scale: [1, 1.3, 1] } : {}}
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="grid flex-1 grid-cols-2 gap-3">
                    {[
                      ...
                      <div
                        key={s.l}
                        className="rounded-lg border border-slate-200/60 bg-gradient-to-br from-slate-50 to-white p-3 text-center shadow-sm"
                      >
        ...
        <motion.main
          variants={panelVariants}
          className="glass-card rounded-2xl p-5 shadow-lg shadow-slate-200/40 sm:p-7"
        >
          <div className="flex h-full flex-col gap-5">
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-slate-400">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <motion.button
                    ...
                    className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-all duration-200 ${
                      isMicOn
                        ? "border-indigo-300 bg-gradient-to-br from-indigo-50 to-violet-50 text-indigo-700 shadow-sm"
                        : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                    }`}
                  >
                    <motion.span
                      animate={isMicOn ? { scale: [1, 1.3, 1] } : {}}
                      transition={{ repeat: Infinity, duration: 1.5 }}
                      className={`h-2 w-2 rounded-full ${isMicOn ? "bg-indigo-500" : "bg-slate-400"}`}
                    />
